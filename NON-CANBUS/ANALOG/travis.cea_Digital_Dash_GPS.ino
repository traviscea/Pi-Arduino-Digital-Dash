...
#define SPEED_SOURCE_MODE 2

/*
0 = VSS only
1 = GPS primary with VSS fallback
2 = GPS only
*/
...
static constexpr float GPS_SPEED_ALPHA = 0.35f;
static constexpr uint32_t GPS_TIMEOUT_MS = 2000;

TinyGPSPlus gps;

static float gpsSpeedKph = 0.0f;
static float gpsSpeedFiltered = NAN;
...
void updateGPS() {
  ...
  if (gps.speed.isUpdated()) {
    ...
    if (raw >= 0.0f && raw < 350.0f) {

      gpsLastFixMs = millis();
      gpsValid = true;

      if (isnan(gpsSpeedFiltered))
        gpsSpeedFiltered = raw;
      else
        gpsSpeedFiltered += GPS_SPEED_ALPHA * (raw - gpsSpeedFiltered);

      gpsSpeedKph = gpsSpeedFiltered;
    }
  }

  if (millis() - gpsLastFixMs > GPS_TIMEOUT_MS)
    gpsValid = false;
}
...
void updateOch(){
  ...
  // Speed (kph)
  float speedSource = getVehicleSpeedKph();

  uint16_t v = (uint16_t)constrain((int)(speedSource + 0.5f), 0, 65535);
  och[19] = v & 0xFF;
  och[20] = v >> 8;
...
void loop(){
  ...
  updateGPS();
  updateVSS();
  updateOdometer(getVehicleSpeedKph());
  shouldSaveOdometer();
